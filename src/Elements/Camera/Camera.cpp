    ...
    const auto sparseWorker = [&](const unsigned threadId) {
        ...
        for (unsigned y = threadId; y < _resolution._y; y += nproc) {
            ...
            for (unsigned x = 0; x < _resolution._x; ++x) {
                ...
                const LightSample mainSample = getRayColor(cameraRay, shapes, lights, config, 0, rng, true, restirGrid[y][x]);
                ...
                for (unsigned c = 1; c < config.antialiasing.samples; ++c) {
                    ...
                    const LightSample sample = getRayColor(cameraRay, shapes, lights, config, 0, rng, true, restirGrid[y][x]);
