...
double raytracer::texture::Perlin::lerp(const double t, const double a,
    const double b) noexcept
{
    return a + t * (b - a);
}

double raytracer::texture::Perlin::grad(const int hash, const double x,
...
{
    const int X = static_cast<int>(std::floor(p._x)) & 255;
    const int Y = static_cast<int>(std::floor(p._y)) & 255;
    const int Z = static_cast<int>(std::floor(p._z)) & 255;
    ...
    const int A = _perm[X] + Y;
    const int AA = _perm[A] + Z;
    const int AB = _perm[A + 1] + Z;
    const int B = _perm[X + 1] + Y;
    const int BA = _perm[B] + Z;
    const int BB = _perm[B + 1] + Z;
    ...
    return lerp(w, 
                lerp(v,
                    ...
                    lerp(u,
                        grad(_perm[AB], x, y - 1, z),
                        grad(_perm[BB], x - 1, y - 1, z))),
                lerp(v,
                    lerp(u,
                        grad(_perm[AA + 1], x, y, z - 1),
